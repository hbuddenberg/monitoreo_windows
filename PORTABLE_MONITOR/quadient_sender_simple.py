...
import time
import smtplib
# Importaciones opcionales con fallback para portabilidad
try:
    import requests
...
class AlertManager:
    ...
    def send_alert(self, title: str, message: str, alert_data: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            # Enviar por webhook
            if self.alert_methods in ['webhook', 'all'] and REQUESTS_AVAILABLE:
            ...
            # Crear alerta en escritorio como respaldo (para versión portable)
            self._create_desktop_alert(title, formatted_message, enriched_data)
    ...
    def _create_desktop_alert(self, title: str, message: str, data: Dict[str, Any]):
        """Crea archivo de alerta en el escritorio como respaldo"""
        try:
            desktop = os.path.expanduser('~/Desktop')
            alert_file = os.path.join(desktop, 'ALERTAS_MONITOR_PORTABLE.txt')
            
            with open(alert_file, 'a', encoding='utf-8') as f:
                f.write(f"\n{'='*60}\n")
                f.write(f"🚨 ALERTA: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"{'='*60}\n")
                f.write(f"Título: {title}\n")
                f.write(f"Mensaje: {message}\n")
                f.write(f"Severidad: {data.get('severity', 'MEDIUM')}\n")
                f.write(f"Equipo: {data.get('hostname', 'N/A')}\n")
                f.write(f"Usuario: {data.get('username', 'N/A')}\n")
                
                if data.get('type') == 'system_critical':
                    f.write(f"Categoría: {data.get('category', 'N/A')}\n")
                    f.write(f"Tiempo arranque: {data.get('boot_time', 'N/A')}\n")
                elif data.get('type') == 'specific_event':
                    f.write(f"Event ID: {data.get('event_id', 'N/A')}\n")
                elif data.get('type') == 'suspicious_process':
                    f.write(f"Proceso: {data.get('process_name', 'N/A')} (PID: {data.get('process_id', 'N/A')})\n")
                    
                f.write(f"\n")
                
        except Exception as e:
            self.logger.error(f"Error creando alerta en escritorio: {e}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'test':
        ...
        test_data = {
            ...
            'severity': 'HIGH',
            'hostname': 'TEST-PC',
            'message': 'Prueba del sistema de alertas portable'
        }
        
        success = alert_manager.send_alert(
            "Prueba Monitor Portable",
            "Este es un mensaje de prueba del sistema de alertas portable",
            test_data
        )
        
        print(f"Prueba de alerta portable: {'EXITOSA' if success else 'FALLIDA'}")
    else:
